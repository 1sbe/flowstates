export interface FlipFluidConfig {
  threshold: number;
...
export class FlipFluid {
  ...
  // configuration used for coloring particles
  ...
  constructor(
    ...
    // initialize particle colors to high color as in original
    for (let i = 0; i < this.maxParticles; i++) {
      this.particleColor[3 * i] = this.config.highr;
      this.particleColor[3 * i + 1] = this.config.highg;
      this.particleColor[3 * i + 2] = this.config.highb;
    }
  }
  ...
  pushParticlesApart(numIters: number) {
    ...
    for (let i = 0; i < this.numParticles; i++) {
      ...
      const xi = this.clamp(Math.floor(x * this.pInvSpacing), 0, this.pNumX - 1);
      const yi = this.clamp(Math.floor(y * this.pInvSpacing), 0, this.pNumY - 1);
  ...
  handleParticleCollisions(obstacleX: number, obstacleY: number, obstacleRadius: number, sceneObstacleVelX: number, sceneObstacleVelY: number) {
    ...
    for (let i = 0; i < this.numParticles; i++) {
      ...
      if (d2 < minDist2) {
        this.particleVel[2 * i] = sceneObstacleVelX;
        this.particleVel[2 * i + 1] = sceneObstacleVelY;
      }
  ...
  updateParticleDensity() {
    ...
    if (this.particleRestDensity === 0.0) {
      ...
      for (let i = 0; i < this.fNumCells; i++) {
        if (this.cellType[i] === 0) { // FLUID_CELL
          sum += d[i];
          numFluidCells++;
        }
      }
  ...
  transferVelocities(toGrid = true, flipRatio = 0.7) {
    const n = this.fNumY;
    const h = this.h;
    const h1 = this.fInvSpacing;
    const h2 = 0.5 * h;
    ...
    if (toGrid) {
      ...
      for (let i = 0; i < this.numParticles; i++) {
        let x = this.particlePos[2 * i];
        let y = this.particlePos[2 * i + 1];
        const xi = this.clamp(Math.floor(x * h1), 0, this.fNumX - 1);
        const yi = this.clamp(Math.floor(y * h1), 0, this.fNumY - 1);
    ...
    for (let iter = 0; iter < numIters; iter++) {
      for (let i = 1; i < this.fNumX - 1; i++) {
        for (let j = 1; j < this.fNumY - 1; j++) {
          const center = i * n + j;
          if (this.cellType[center] !== 0 /*FLUID_CELL*/) continue;
          ...
          this.u[center] -= sx0 * pVal;
          this.u[right] += sx1 * pVal;
          this.v[center] -= sy0 * pVal;
          this.v[top] += sy1 * pVal;
        }
      }
    }
  ...
  updateParticleColors() {
    ...
    for (let i = 0; i < this.numParticles; i++) {
      this.particleColor[3 * i] = this.clamp(this.particleColor[3 * i], 0.0, 1.0);
      this.particleColor[3 * i + 1] = this.clamp(this.particleColor[3 * i + 1], 0.0, 1.0);
      this.particleColor[3 * i + 2] = this.clamp(this.particleColor[3 * i + 2], 0.0, 1.0);

      const x = this.particlePos[2 * i];
      const y = this.particlePos[2 * i + 1];
      const xi = this.clamp(Math.floor(x * h1), 1, this.fNumX - 1);
      const yi = this.clamp(Math.floor(y * h1), 1, this.fNumY - 1);
      const cellNr = xi * this.fNumY + yi;

      const d0 = this.particleRestDensity;
      if (d0 > 0.0) {
        const relDensity = this.particleDensity[cellNr] / d0;
        if (relDensity < threshold) {
          this.particleColor[3 * i] = this.config.lowr;
          this.particleColor[3 * i + 1] = this.config.lowg;
          this.particleColor[3 * i + 2] = this.config.lowb;
        } else {
          this.particleColor[3 * i] = this.config.highr;
          this.particleColor[3 * i + 1] = this.config.highg;
          this.particleColor[3 * i + 2] = this.config.highb;
  ...
  simulate(
    ...
    for (let step = 0; step < numSubSteps; step++) {
      ...
      this.handleParticleCollisions(obstacleX, obstacleY, obstacleRadius, 0, 0); // vel set by scene elsewhere
